# modified from https://github.com/cmhcbb/attackbox/blob/master/attack/Sign_OPT.py
...
class OPT_attack_sign_SGD(object):
    def __init__(self, model, backup_starting_points, k=200, train_dataset=None, ):
        self.backup_starting_points = backup_starting_points
        self.model = model
        self.k = k
        self.train_dataset = train_dataset
        self.log = np.ones((MAX_ITER, 2), dtype=np.float32)
    ...
    def attack_untargeted(self, x0, y0, alpha=0.2, beta=0.001, iterations=1000, query_limit=20000,
        ...
        if (yield from model.predict_label(x0)) != y0:
        ...
        for i in range(num_directions):
            ...
            if (yield from model.predict_label(x0 + theta.astype(np.float32))) != y0:
                initial_lbd = LA.norm(theta)
                theta /= initial_lbd  # l2 normalize
                lbd, count = yield from self.fine_grained_binary_search(model, x0, y0, theta, initial_lbd, g_theta)
                query_count += count
                if lbd < g_theta:
        ...
        if g_theta == float('inf'):
            ...
            while True:
                query_count += 1
                classes = list(self.backup_starting_points.keys())
                if y0 in classes: classes.remove(y0)
                starting_point = self.backup_starting_points[np.random.choice(classes)]
                # register adv directions
                theta = starting_point - x0
                assert (yield from model.predict_label(x0 + theta.astype(np.float32))) != y0
                initial_lbd = LA.norm(theta)
                theta /= initial_lbd  # l2 normalize
                lbd, count = yield from self.fine_grained_binary_search(model, x0, y0, theta, initial_lbd, g_theta)
                query_count += count
        ...
        for i in range(iterations):
            ...
            if svm == True:
                sign_gradient, grad_queries = yield from self.sign_grad_svm(x0, y0, xg, initial_lbd=gg, h=beta)
            else:
                sign_gradient, grad_queries = yield from self.sign_grad_v1(x0, y0, xg, initial_lbd=gg, h=beta)
            ...
            for _ in range(15):
                ...
                new_g2, count = yield from self.fine_grained_binary_search_local(
    ...
    def sign_grad_v1(self, x0, y0, theta, initial_lbd, h=0.001, D=4, target=None):
        ...
        for iii in range(K):  # for each u
            ...
            if (target is not None and
                    (yield from self.model.predict_label(x0 + initial_lbd * new_theta)) == target):
                sign = -1

            # Untargeted case
            # preds.append(self.model.predict_label(x0+xxxx.tensor(initial_lbd*new_theta, dtype=xxxx.float).cuda()).item())
            if (target is None and
                    (yield from self.model.predict_label(x0 + initial_lbd * new_theta)) != y0):  # success
                sign = -1
    ...
    def sign_grad_v2(self, x0, y0, theta, initial_lbd, h=0.001, K=200):
        ...
        for _ in range(K):
            ...
            new_theta = theta + h * u
            new_theta /= LA.norm(new_theta)
            if (yield from self.model.predict_label(x0 + initial_lbd * new_theta)) == y0:
                ss = 1
            queries += 1
            sign_grad += sign(u) * ss
        sign_grad /= K
    ...
    def sign_grad_svm(self, x0, y0, theta, initial_lbd, h=0.001, K=100, lr=5.0, target=None):
        ...
        for iii in range(K):
            ...
            # Untargeted case
            if (target is None and
                    (yield from self.model.predict_label(x0 + initial_lbd * new_theta)) != y0):
                sign = -1
    ...
    def fine_grained_binary_search_local(self, model, x0, y0, theta, initial_lbd=1.0, tol=1e-5):
        ...
        if (yield from model.predict_label(x0 + lbd * theta)) == y0:
            lbd_lo = lbd
            lbd_hi = lbd * 1.01
            nquery += 1
            while (yield from model.predict_label(x0 + lbd_hi * theta)) == y0:
                lbd_hi = lbd_hi * 1.01
                nquery += 1
                if lbd_hi > 20:
            ...
            lbd_lo = lbd * 0.99
            nquery += 1
            while (yield from model.predict_label(x0 + lbd_lo * theta)) != y0:
                lbd_lo = lbd_lo * 0.99
                nquery += 1
        ...
        while (lbd_hi - lbd_lo) > tol:
            ...
            nquery += 1
            if (yield from model.predict_label(x0 + lbd_mid * theta)) != y0:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
    ...
    def fine_grained_binary_search(self, model, x0, y0, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best:
            if (yield from model.predict_label(x0 + current_best * theta)) == y0:
                nquery += 1
                return float('inf'), nquery
            lbd = current_best
        else:
