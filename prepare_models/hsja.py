# modified from https://github.com/Jianbo-Lab/HSJA/blob/master/hsja.py
...
def hsja(model,
    ...
    # Initialize.
    perturbed = yield from initialize(model, sample, params, backup_starting_points)

    # Project the initialization to the boundary.
    perturbed, dist_post_update = yield from binary_search_batch(sample,
    ...
    for j in np.arange(params['num_iterations']):
        ...
        print("num_evals = {}".format(num_evals))

        # approximate gradient.
        gradf = yield from approximate_gradient(model, perturbed, num_evals,
                                     delta, params)
        ...
        if params['stepsize_search'] == 'geometric_progression':
            ...
            idx_perturbed = yield from decision_function(model, perturbeds, params)
...
def decision_function(model, images, params):
    ...
    labels = yield from model.predict_labels(images)
    if params['target_label'] is None:
        return labels != params['original_label']
    else:
        return labels == params['target_label']
...
def approximate_gradient(model, sample, num_evals, delta, params):
    ...
    # query the model.
    decisions = yield from decision_function(model, perturbed, params)
...
def binary_search_batch(original_image, perturbed_images, model, params):
    ...
    while np.max((highs - lows) / thresholds) > 1:
        ...
        decisions = yield from decision_function(model, mid_images, params)
...
def initialize(model, sample, params, backup_starting_points):
    """
    ...
    if params['target_image'] is None:
        ...
        while True:
            ...
            success = (yield from decision_function(model, random_noise[None], params))[0]
            num_evals += 1
            if success:
                break
            if num_evals == 101:
                classes = list(backup_starting_points.keys())
                if params['original_label'] in classes: classes.remove(params['original_label'])
                random_noise = backup_starting_points[np.random.choice(classes)]
                random_noise = random_noise.reshape(params['shape'])
                break
        ...
        while high - low > 0.001:
            ...
            success = yield from decision_function(model, blended[None], params)
            if success:
                high = mid
            else:
                low = mid
...
def geometric_progression_for_stepsize(x, update, dist, model, params):
    ...
    def phi(epsilon):
        ...
        success = yield from decision_function(model, new[None], params)
        return success
